...
| Role | Scope |
|---|---|
| **Senior Manager** | All sections and units — org-wide overview, the audio briefing (exclusive), blocked/delayed alerts |
| **Section Head** | Their section only — every unit under it, workloads, statistics |
| **Unit Head** | Their own unit — member workloads, reminders, reassignment |
| **Team Member** | Their own tasks only — manual, AI chat, or voice updates; no org pages |
...
- **Profile & preferences** — every user has a profile page with a saved **preferred language and theme**, stored per user in the database; the header language/theme toggles save there too, so preferences follow each user across sign-ins and devices.
...
- **Audio briefing** *(senior manager only)* — a daily update spoken **the way a manager reports to their senior**: first person and accountable — "here's where we stand", the wins first, "the items where I need your help" with the person and the reason, slipped deadlines with the corrective action, a roundup across the units, and one closing ask. Generated fresh from live data server-side, spoken on-device (Web Speech), in both languages, with section/unit scope selection. A **3D presenter avatar** (three.js) fronts the player: she blinks, smiles, and sways when idle, and while the briefing plays her mouth articulates **in sync with the audio** — every word boundary reported by the speech engine fires a viseme pulse, so the lips track the actual narration cadence (with a rhythmic fallback for voices that don't report boundaries); an illustrated portrait takes over wherever WebGL isn't available. The avatar is a Ready Player Me model from the MIT-licensed [TalkingHead](https://github.com/met4citizen/TalkingHead) project, bundled at `public/models/avatar.glb`.
...
- **Sections → Units → Members** — the org drill-down (demo: *Business Excellence* with Data Management & Business Development; *Corporate Governance* with Unit 1 & Unit 2), with unit health badges (Healthy / Watch / At risk), member workload bars, and per-unit CSV export. Visibility follows the hierarchy strictly: senior manager sees everything, a section head only their section, a unit head only their unit, and team members see no org pages at all.
