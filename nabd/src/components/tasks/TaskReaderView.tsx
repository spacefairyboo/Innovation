    ...
    >
      ...
      <span className='absolute inset-0 grid place-items-center rotate-0'>
        <span className='text-center'>
          <span className='block text-3xl font-bold text-white tabular-nums leading-none'>
            {pct}%
          </span>
        </span>
      </span>
  ...
  return (
    <div className='flex items-start gap-3.5 py-3 border-b border-grid last:border-b-0'>
      ...
      <div className='flex-1 min-w-0'>
        <div className='text-[0.68rem] font-bold uppercase tracking-wide text-ink-3'>
          {label}
        </div>
        <div
          className={`text-sm font-semibold mt-0.5 break-words ${warn ? 'text-[var(--st-delayed)]' : 'text-ink'}`}
        >
          {children}
        </div>
      </div>
...
export function TaskReaderView({
  vm,
  backHref,
}: {
  vm: TaskVM;
      ...
      >
        ...
        <div className='relative flex items-center gap-6 flex-wrap'>
          <div className='flex-1 min-w-64'>
            ...
            <h2 className='m-0 mt-3 text-3xl font-bold text-white leading-snug'>
              {task.title[lang]}
            </h2>
            <div className='mt-3.5 flex items-center gap-2 flex-wrap'>
              <HeroPill icon={STATUS_META[eff].icon} tone={`var(--st-${eff})`}>
                <span className='text-white'>
                  {t(STATUS_META[eff].labelKey)}
                </span>
              </HeroPill>
              {vm.value.high && (
                <HeroPill icon='sparkles'>{t('value_high')}</HeroPill>
              )}
            </div>
          </div>
      ...
      <div className='grid gap-5 lg:[grid-template-columns:1.55fr_1fr] *:min-w-0 items-start'>
        <div className='grid gap-5 min-w-0'>
          ...
          <div className='card'>
            ...
            {task.description ? (
              <p className='m-0 text-[0.95rem] leading-7 text-ink-2 whitespace-pre-wrap'>
                {task.description}
              </p>
            ) : (
              <p className='m-0 text-sm text-ink-3 italic'>
                {t('reader_no_desc')}
              </p>
            )}
          </div>
          ...
          <div className='card !py-2.5'>
              ...
              <span className='flex items-center gap-2 flex-wrap'>
                <span className='flex -space-x-1.5 rtl:space-x-reverse'>
                  {vm.assignees.slice(0, 4).map((a) => (
                    <span
                      key={a.id}
                      className='ring-2 ring-[var(--surface-solid)] rounded-full'
                    >
            ...
            {vm.assignees[0]?.managerName && (
              <DetailRow icon='users' label={t('line_manager')}>
                {vm.assignees[0].managerName[lang]}
              </DetailRow>
            )}
            <DetailRow icon='flag' label={t('priority')}>
              {t(PRIO_KEY[task.priority])}
            </DetailRow>
            {task.due && (
              <DetailRow
                icon='calendar'
                label={t('due_date')}
                warn={dueView.overdue}
              >
                {task.due}
            ...
            {vm.projectName && (
              <DetailRow icon='folder' label={t('project')}>
                {vm.projectName}
              </DetailRow>
            )}
            {task.tags.length > 0 && (
              <DetailRow icon='tag' label={t('tags')}>
                <span className='flex gap-1.5 flex-wrap'>
                  {task.tags.map((x) => (
                    <span
                      key={x}
                      className='rounded-full bg-accent-soft text-primary px-2.5 py-0.5 text-xs font-semibold'
                    >
                      #{x}
                    </span>
                  ))}
            ...
            <DetailRow icon='history' label={t('updated')}>
              {relTime(task.updatedAt, t)}
            </DetailRow>
          </div>
        </div>
