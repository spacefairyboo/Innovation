...
export function PodcastPlayer({
  ...
  const [spoken, setSpoken] = useState<'en' | 'ar'>(
    lang === 'ar' ? 'ar' : 'en',
  );
  ...
  const baseSec = (i: number) =>
    words.slice(0, i).reduce((a, b) => a + b, 0) * secPerWord;
  ...
  const playLine = (i: number, atRate = rate, atVoiceURI = voiceURI) => {
    ...
    setLineIdx(clamped);
    setElapsed(
      words.slice(0, clamped).reduce((a, b) => a + b, 0) *
        (60 / (160 * atRate)),
    );
    const utt = new SpeechSynthesisUtterance(lines[clamped]);
    utt.lang = spoken === 'ar' ? 'ar-SA' : 'en-US';
    const voice =
      voices.find((v) => v.voiceURI === atVoiceURI) ?? voices[0] ?? null;
  ...
  return (
    <>
      ...
      >
        ...
        <span
          aria-hidden
          className='absolute -bottom-28 start-1/3 w-80 h-80 rounded-full pointer-events-none'
          style={{ background: 'rgb(37 150 190 / 0.14)', filter: 'blur(80px)' }}
        />
        ...
        <div className='relative flex items-center gap-4 flex-wrap'>
          <button
            ...
            style={{
              background: 'rgb(70 199 180 / 0.16)',
              border: '1.5px solid rgb(70 199 180 / 0.55)',
              color: '#5cd6c4',
            }}
            onClick={toggle}
          ...
          <div className='flex-1 min-w-44'>
            ...
            <p className='m-0 mt-1 text-sm' style={{ color: '#9cc4ba' }}>
              {t('today')} · {Math.max(1, Math.round(totalSec / 60))}{' '}
              {t('podcast_min')}
              {highlights > 0 && (
                <>
                  {' '}
                  · {highlights} {t('podcast_highlights')}
                </>
              )}
            </p>
          </div>
        ...
        <div
          ...
          onKeyDown={(e) => {
            if (e.key === 'ArrowRight')
              playLine(Math.min(lines.length - 1, trackIdx + 1));
          ...
          {Array.from({ length: BARS }, (_, i) => (
            <span
              ...
              style={{
                height: barHeight(i),
                background:
                  i < playedBars ? '#46c7b4' : 'rgb(223 245 241 / 0.16)',
              }}
            />
          ))}
        </div>
        ...
        <div
          dir='ltr'
          className='relative flex items-center gap-3 mt-2 text-xs'
          style={{ color: '#9cc4ba' }}
        >
          <span className='tabular-nums shrink-0'>{fmt(elapsed)}</span>
          <span
            className='flex-1 text-center truncate'
            style={{ color: playing ? '#d9efe9' : '#7fa89e' }}
          >
            {quote ? `“${quote}”` : ''}
          </span>
        ...
        <div className='relative flex items-center gap-2.5 mt-5 flex-wrap'>
          ...
          <div
            className='inline-flex rounded-xl overflow-hidden border border-white/20'
            role='group'
            aria-label={t('podcast_lang')}
          >
      ...
      <div className='card mt-4.5'>
        <div className='flex items-center gap-2.5 mb-3'>
          ...
          <button className='btn-ghost btn-sm' onClick={download}>
            <Icon name='download' size={13} /> {t('download_script')}
          </button>
        </div>
        <div
          ref={transcriptRef}
          dir={spoken === 'ar' ? 'rtl' : 'ltr'}
          className='text-sm text-ink-2 leading-7 max-h-104 overflow-y-auto pe-2'
        >
          {lines.map((l, i) => (
            <p
              key={i}
              data-line={i}
              className={`m-0 mb-3 ${i === lineIdx ? 'text-primary font-semibold' : ''}`}
            >
              {l}
            </p>
          ))}
