...
export function Modal({ title, icon, onClose, children, footer }: {
  ...
  return (
    <div
      className="fixed inset-0 z-90 grid place-items-center p-5 bg-[rgb(4_22_30/0.55)] backdrop-blur-xs"
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="bg-surface rounded-2xl shadow-2xl w-full max-w-xl max-h-[88vh] flex flex-col overflow-hidden animate-modal-pop">
        <div className="flex items-center gap-2.5 px-5 py-4 border-b border-line">
          ...
          <button className="icon-btn !w-8 !h-8" onClick={onClose} aria-label="close">✕</button>
        </div>
...
export function dueInfo(due: string | null, t: (k: string, v?: Record<string, string | number>) => string, lang: string):
  ...
  if (d < 0) return { text: `⚠️ ${t("overdue")} · ${Math.abs(d)}${lang === "ar" ? " يوم" : "d"}`, overdue: true };
