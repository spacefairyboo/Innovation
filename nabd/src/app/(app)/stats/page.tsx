...
import {
  ArcChart, BreakdownTable, BucketTable, ChartCard, ColumnChart,
  Donut, LineChart, ProgressBars, ProgressTable, StatTiles,
...
import {
  ...
  listUsers, teamMembers, teamTasks, userTasks,
} from "@/server/repositories";
import { getSession } from "@/server/auth/session";
import {
  DAY_MS, countStatuses, effStatus, teamHealth, todayISO,
  type EffStatus, type Task,
} from "@/lib/types";
import { completionTrend, csvRows, doneThisWeekCount, topContributors } from "@/server/vm";
...
export default async function StatsPage({ searchParams }: {
  ...
  // The department head reads the org at section level; focusing a section
  // (or being a section head) drops one level to its units.
  const seniorAll = user.role === "senior" && !focus;
  const groups = seniorAll
    ? {
        groupLabel: t("unit"),
        buckets: listUnits().map((sec) => ({
          id: sec.id, label: sec.name[lang], tasks: sectionTasks(sec.id).filter(inView),
          head: listUsers().find((u) => u.role === "section" && u.sectionId === sec.id)?.name[lang] ?? null,
        })),
      }
    : byUnits
    ? (() => {
      ...
      {view === "projects" ? (
        ...
        <>
          ...
          <div className="grid gap-5 lg:grid-cols-2 mb-5">
            ...
            />
            <ChartCard
              title={user.role === "manager" ? t("by_member") : seniorAll ? t("by_section") : t("by_team")}
              sub={t("by_team_sub")}
